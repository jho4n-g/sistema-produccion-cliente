...
import { getObj } from '../../../../../service/Produccion/Secciones/Barbotina.services';
      ...
      <div>
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-xl font-semibold text">
            Produccion / Control moliento barbotina
          </h2>
        </div>
          ...
          <table className="w-full table-fixed border-separate border-spacing-0 text-sm">
            <thead className="bg-slate-50">
              <tr className="divide-x divide-slate-200">
                <th className="w-12 border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">
                  N°
                </th>
                <th className="w-28 border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">
                  Fecha
                </th>
                <th className="w-20 border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">
                  Turno
                </th>
                <th className="w-48 border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">
                  Operador
                </th>
                <th className="w-28 border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">
                  Equipo
                </th>
                <th className="w-32 border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">
                  Horómetro inicio
                </th>
                <th className="w-32 border-b border-slate-200 px-4 py-3 text-left font-semibold text-slate-700">
                  Horómetro final
              ...
              {loading ? (
                ...
                paginated.map((data, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-slate-700 wrap-break-word  ">
                      {data.fecha}
                    </td>
                    <td className="px-4 py-3 text-slate-700  ">Segundo</td>
                    <td className="px-4 py-3 text-slate-900 wrap-break-word whitespace-normal">
                      {data.operador}
                    </td>
                    <td className="px-4 py-3 text-slate-900 wrap-break-word whitespace-normal">
                      {data.equipo}
                    ...
                    <td className="px-4 py-3 text-slate-700">
                      {data?.ObservacionesBarbotinaDatos?.map(
                        (o) => o.observacion
                      ).join(' | ')}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-2">
                        <button className="rounded-xl bg-green-800 px-3 py-2 text-white hover:bg-green-900">
                          Editar
                        </button>
                        <button
