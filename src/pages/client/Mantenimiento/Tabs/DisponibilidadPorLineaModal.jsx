import { useState, useEffect } from 'react';
import { DatosDisponibilidaPorLinea } from '@schema/mantenimiento/DisponibilidadPorLinea.schema';
import InputField from '@components/InputField';
import { toast } from 'react-toastify';
const initialForm = () => ({
  fecha: '',
  n_horas_productivas_planificadas: '',
  n_horas_lineas_paradas_linea_b: '',
  n_horas_lineas_paradas_linea_c: '',
  n_horas_lineas_paradas_linea_d: '',
  n_horas_lineas_paradas_linea_e: '',
});

export default function DisponibilidadPorLineaModal({
  open,
  onClose,
  onSave,
  fetchById,
  id,
  isEdit = false,
}) {
  const [form, setForm] = useState();
  const [error, setError] = useState({});
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (!open || !id) return; // evita correr si no aplica

    let active = true; // evita setState tras unmount
    setLoading(true);

    // CREAR
    if (!isEdit) {
      setForm(initialForm());
      setError({});
      setLoading(false);
      return () => {
        active = false;
      };
    }

    // EDITAR
    if (!id) {
      setLoading(false);
      return () => {
        active = false;
      };
    }

    (async () => {
      try {
        const data = await fetchById(id); // ← ahora sí esperamos aquí

        if (!active) return;

        if (data?.ok) {
          setForm(data.dato ?? {});
        } else {
          toast.error(data?.message || 'No se pudo cargar el registro');
        }
      } catch (e) {
        if (active) toast.error(e?.message || 'Error del servidor');
      } finally {
        if (active) setLoading(false); // ← se apaga al terminar de verdadfi
      }
    })();

    return () => {
      active = false;
    };
  }, [open, id, fetchById, isEdit]);

  if (!open) return null;

  const updateBase = (e) => {
    const { name, value } = e.target;
    setForm((f) => ({ ...f, [name]: value }));
    setError((prev) => ({ ...prev, [name]: undefined }));
  };

  const handleValidation = async () => {
    const result = DatosDisponibilidaPorLinea.safeParse(form);
    if (!result.success) {
      const { fieldErrors } = result.error.flatten();

      setError(fieldErrors);
      toast.error('Datos incorrectos');
      return;
    } else {
      const data = result.data;
      onSave(data);
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      {/* Overlay (fondo) */}
      <div
        onClick={loading ? undefined : onClose}
        className="absolute inset-0 bg-black/40"
      />
      <div
        className="relative z-10 w-3xl max-w-7xl rounded-2xl bg-white shadow-xl ring-1 ring-slate-200
                max-h-[calc(100vh-2rem)] overflow-y-auto"
      >
        {loading && (
          <div className="absolute inset-0 z-50 grid place-items-center rounded-2xl bg-white/75 backdrop-blur">
            <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow">
              <span className="h-5 w-5 animate-spin rounded-full border-2 border-slate-300 border-t-slate-900" />
              <p className="text-sm font-semibold text-slate-800">
                Cargando datos…
              </p>
            </div>
          </div>
        )}

        {!loading && (
          <>
            <div className="flex items-start justify-between border-b border-slate-200 px-5 py-4">
              <h3 className="text-lg font-semibold text-slate-900">
                Disponibilidad por linea
              </h3>
            </div>

            <div className="bg-white rounded-xl shadow p-4 sm:p-6 mb-2">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 sm:gap-6">
                {/* Fila 1 */}
                <div className="md:col-span-1 lg:col-span-3">
                  <InputField
                    label="fFcha"
                    type="date"
                    name="fecha"
                    value={form?.fecha || ''}
                    onChange={updateBase}
                    error={error.fecha}
                  />
                </div>

                {/* Fila 2 */}
                <div className="md:col-span-1 lg:col-span-6">
                  <InputField
                    label="N° horas productivas planificadas"
                    type="number"
                    name="n_horas_productivas_planificadas"
                    value={form?.n_horas_productivas_planificadas || ''}
                    onChange={updateBase}
                    error={error.n_horas_productivas_planificadas}
                  />
                </div>

                <div className="md:col-span-1 lg:col-span-6">
                  <InputField
                    label="N° horas lineas paradas linea b"
                    type="number"
                    name="n_horas_lineas_paradas_linea_b"
                    value={form?.n_horas_lineas_paradas_linea_b || ''}
                    onChange={updateBase}
                    error={error.n_horas_lineas_paradas_linea_b}
                  />
                </div>
                <div className="md:col-span-1 lg:col-span-6">
                  <InputField
                    label="N° horas lineas paradas linea c"
                    type="number"
                    name="n_horas_lineas_paradas_linea_c"
                    value={form?.n_horas_lineas_paradas_linea_c || ''}
                    onChange={updateBase}
                    error={error.n_horas_lineas_paradas_linea_c}
                  />
                </div>
                <div className="md:col-span-1 lg:col-span-6">
                  <InputField
                    label="N° horas lineas paradas linea d"
                    type="number"
                    name="n_horas_lineas_paradas_linea_d"
                    value={form?.n_horas_lineas_paradas_linea_d || ''}
                    onChange={updateBase}
                    error={error.n_horas_lineas_paradas_linea_d}
                  />
                </div>
                <div className="md:col-span-1 lg:col-span-6">
                  <InputField
                    label="N° horas lineas paradas linea e"
                    type="number"
                    name="n_horas_lineas_paradas_linea_e"
                    value={form?.n_horas_lineas_paradas_linea_e || ''}
                    onChange={updateBase}
                    error={error.n_horas_lineas_paradas_linea_e}
                  />
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-2 p-5">
              <button
                className="rounded-xl bg-red-800 px-3 py-2 text-white hover:bg-red-900"
                onClick={onClose}
              >
                Cancelar
              </button>
              <button
                className="rounded-xl bg-green-800 px-3 py-2 text-white hover:bg-green-900"
                onClick={handleValidation}
              >
                Guardar cambios
              </button>
            </div>
          </>
        )}
      </div>
    </div>
  );
}
