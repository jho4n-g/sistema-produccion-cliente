...
//
import { registerObj } from '@service/Administracion/Donaciones.services';
...
export default function CalidadLlenado() {
  ...
  const handleValidation = async () => {
    const result = DatosDonaciones.safeParse(form);
    if (!result.success) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 sm:gap-6">
          ...
          <div className="md:col-span-1 lg:col-span-6">
            <InputField
              ...
              type="number"
              name="produccion_menual"
              value={form?.produccion_menual || ''}
              onChange={updateBase}
              error={error.produccion_menual}
            />
          </div>

          <div className="md:col-span-1 lg:col-span-6">
            <InputField
              label="Cascote mensual"
              type="number"
              name="cascote_mensual"
              value={form?.cascote_mensual || ''}
              onChange={updateBase}
              error={error.cascote_mensual}
            />
          </div>
          <div className="md:col-span-1 lg:col-span-6">
            <InputField
